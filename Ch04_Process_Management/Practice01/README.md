...
이번에는 `wait(0)`의 주석 처리를 지워보고 위 코드를 돌려보면, 몇 번을 돌려도 "I am child process. ..."들이 먼저 출력되고 나서 차례로 "I am parent process. ..."들이 출력됩니다. 부모 프로세스가 자식 프로세스가 종료될 때까지 기다리기 때문입니다. 따라서 `wait(0)`을 쓴다면 하림이처럼 세상이 거꾸로 돌아가는 일을 막고 프로세스의 순서를 제어할 수 있습니다.
